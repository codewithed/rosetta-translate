...
const ConversationScreen: React.FC = () => {
  ...
  const ensureRecordingDirectory = async () => {
    ...
    console.log(
      "[ConversationScreen] ensureRecordingDirectory: Target recording directory:",
      recordingDir
    );
    ...
    if (!dirInfo.exists) {
      console.log(
        "[ConversationScreen] ensureRecordingDirectory: Recording directory does not exist. Creating it..."
      );
      try {
        ...
        console.log(
          "[ConversationScreen] ensureRecordingDirectory: Recording directory created successfully."
        );
      } catch (error) {
      ...
      console.log(
        "[ConversationScreen] ensureRecordingDirectory: Recording directory already exists."
      );
    }
  };
  ...
  const handleCopyToClipboard = async (text: string, type: string) => {
    ...
    if (text && text.trim() !== "") {
      ...
      Alert.alert(
        "Nothing to Copy",
        `There is no ${type.toLowerCase()} to copy.`
      );
    }
  };
  ...
  const processTurn = async (
    ...
    try {
      if (audioUri) {
        console.log(
          "[ConversationScreen] processTurn: Received audioUri:",
          audioUri
        );
        ...
        console.log(
          "[ConversationScreen] processTurn: Waiting for file to be ready..."
        );
        ...
        if (!fileInfo.exists || fileInfo.size === 0) {
          ...
          Alert.alert(
            "File Error",
            `Recording file not found or is empty. Please try recording again.`
          );
          setIsLoading(false);
          return;
        }
        ...
        if (
          !sttResponse.success ||
          typeof sttResponse.message !== "string" ||
          !sttResponse.message
        ) {
      ...
      const translateResponse = await apiTranslateText({
        text: currentOriginalText,
        sourceLang: sourceLangBase,
        targetLang: targetLangBase,
      });
      if (
        !translateResponse.success ||
        typeof translateResponse.message !== "string"
      ) {
        throw new Error(translateResponse.message || "Translation failed.");
      }
      const translatedText = translateResponse.message;

      const ttsResponse = await apiTextToSpeech({
        text: translatedText,
        languageCode: targetLangWithRegion,
      });
      if (!ttsResponse.success || !ttsResponse.data) {
        throw new Error(
          ttsResponse.message ||
            "Text-to-speech failed or returned no audio data."
        );
      }
  ...
  const handleRecord = async (speaker: "userA" | "userB") => {
    ...
    if (isRecording) {
      try {
        ...
        console.log(
          "[ConversationScreen] handleRecord: Recording stopped. URI:",
          uri
        );
        if (speaker === "userA") setIsRecordingA(false);
        ...
        if (uri && typeof uri === "string" && uri.trim()) {
          ...
          Alert.alert(
            "Recording Error",
            "Failed to get a valid audio recording file."
          );
        }
      } catch (err: any) {
      ...
      if (audioRecorder.isRecording) {
        try {
          console.warn(
            "[ConversationScreen] handleRecord: Found an ongoing recording. Stopping it first."
          );
          await audioRecorder.stop();
        } catch (stopErr: any) {
          console.warn(
            "[ConversationScreen] handleRecord: Error stopping previous recording before starting new one:",
            stopErr
          );
        }
      }
      try {
        console.log(
          "[ConversationScreen] handleRecord: Ensuring recording directory..."
        );
        await ensureRecordingDirectory();

        console.log(
          "[ConversationScreen] handleRecord: Preparing to record..."
        );
        ...
        if (speaker === "userA") setIsRecordingA(true);
  ...
  return (
    ...
    >
      <View style={styles.languageSelectorRow}>
        <TouchableOpacity
          onPress={() => {
            setLanguageModalType("userA");
            setIsLanguageModalVisible(true);
          }}
          style={styles.languageButton}
        >
          <Text style={styles.languageButtonText}>
            {langA.split("-")[0].toUpperCase()}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        <TouchableOpacity
          onPress={() => {
            setLanguageModalType("userB");
            setIsLanguageModalVisible(true);
          }}
          style={styles.languageButton}
        >
          <Text style={styles.languageButtonText}>
            {langB.split("-")[0].toUpperCase()}
          </Text>
        </TouchableOpacity>
      </View>
        ...
        {conversationTurns.map((turn) => (
          <View
            ...
            style={[
              ...
              turn.speaker === "userA"
                ? styles.userAContainer
                : styles.userBContainer,
            ]}
          >
            <View style={styles.textBubble}>
              ...
              <View style={styles.bubbleActions}>
                <TouchableOpacity
                  onPress={() =>
                    handleCopyToClipboard(turn.translatedText, "Translation")
                  }
                >
      ...
      <View style={styles.inputContainer}>
        <View style={[styles.inputWrapper, styles.inputWrapperA]}>
          ...
          />
          <TouchableOpacity
            onPress={() => handleRecord("userA")}
            style={styles.micButton}
          >
            <Ionicons
              name="mic"
              size={24}
              color={isRecordingA ? "red" : "#007AFF"}
            />
          </TouchableOpacity>
        </View>
        <View style={[styles.inputWrapper, styles.inputWrapperB]}>
          ...
          />
          <TouchableOpacity
            onPress={() => handleRecord("userB")}
            style={styles.micButton}
          >
            <Ionicons
              name="mic"
              size={24}
              color={isRecordingB ? "red" : "#007AFF"}
            />
          </TouchableOpacity>
        </View>
      </View>
