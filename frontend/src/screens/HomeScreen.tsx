...
import { Camera, PermissionStatus as CameraPermissionStatus, PermissionResponse as CameraPermissionResponse } from 'expo-camera';
...
const HomeScreen: React.FC = () => {
    ...
    const [itemToSave, setItemToSave] = useState<ApiTranslationResponse | null>(null);
    ...
    useEffect(() => {
        ...
        if (user?.preferences) {
            if (user.preferences.preferredSourceLang) setSourceLang(user.preferences.preferredSourceLang);
            if (user.preferences.preferredTargetLang) setTargetLang(user.preferences.preferredTargetLang);
        }
    }, [user]);
    ...
    const handleApiTranslationSuccess = async (original: string, translated: string, from: string, to: string, inputType: InputType) => {
    ...
    const handleSpeechToText = async (uri: string) => {
        ...
        try {
            ...
            const fileData = {
                uri: uri,
                name: uri.split('/').pop() || Platform.OS === 'ios' ? 'recording.wav' : 'recording.amr',
    ...
    try {
        ...
        if (apiResponse.success && apiResponse.message === "TTS_AUDIO_GENERATED" && typeof apiResponse.data === 'string') {
            ...
            try {
                ...
                console.log(`Audio file created successfully at: ${tempAudioUri}, size: ${fileInfo.size} bytes`);

                // Clean up previous audio file if it exists
                if (currentPlaybackUri && currentPlaybackUri !== tempAudioUri) {
                    try {
                        const oldFileInfo = await FileSystem.getInfoAsync(currentPlaybackUri);
                        if (oldFileInfo.exists) {
                            await FileSystem.deleteAsync(currentPlaybackUri, { idempotent: true });
   ...
   const handleCreateNewFolderInPopup = () => {
    Alert.prompt("New Folder", "Enter a name for your new folder:", async (folderName) => {
        if (folderName) {
            try {
                setIsFoldersLoading(true); // Show loading during creation
                const newFolder = await createNewFolder(folderName);
                
                // Ensure the folder is fully created before updating state
    ...
    const startRecording = async () => {
        try {
            ...
            setIsRecording(true);
        } catch (err: any) { Alert.alert('Recording Error', `Failed to start recording: ${err.message}`); }
    };
    ...
    const stopRecording = async () => {
        ...
        } catch (error: any) { Alert.alert('Error', `Failed to stop recording: ${error.message}`); }
    };

    const handleAudioRecord = () => isRecording ? stopRecording() : startRecording();
    ...
    const prepareForMediaInput = () => {
        setInputText(''); setResultText(''); setCurrentApiTranslation(null);
        setSelectedImage(null); setAudioUri(null);
    };
                    ...
                    {resultText && !isLoading && (
                        <View style={styles.resultDisplayCard}>
                            <View style={styles.resultHeaderActions}>
                                ...
                                <View style={styles.resultIcons}>
                                    <TouchableOpacity onPress={() => handleTextToSpeech(resultText, targetLang)} style={styles.iconButton}>
                                    ...
                                    {currentApiTranslation && (
                                        <>
                                            ...
                                            <TouchableOpacity onPress={() => openSaveModal(currentApiTranslation)} style={styles.iconButton}>
                                                <Ionicons name="bookmark-outline" size={24} color={theme.colors.primaryOrange} />
                                            </TouchableOpacity>
                                        </>
                                    )}
                                </View>
...
const styles = StyleSheet.create({
    ...
    translationCard: { backgroundColor: theme.colors.cardBackground, borderRadius: theme.borders.radiusLarge, marginHorizontal: theme.spacing.md, marginTop: theme.spacing.lg, padding: theme.spacing.md, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 3, },
    languageSelectorRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: theme.spacing.md, },
    languageButton: { paddingHorizontal: theme.spacing.md, paddingVertical: theme.spacing.sm, backgroundColor: theme.colors.lightGray, borderRadius: theme.borders.radiusMedium, },
    languageButtonText: { ...theme.typography.button, color: theme.colors.primaryOrange, textTransform: 'uppercase', },
    inputHeaderActions: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: theme.spacing.xs, paddingHorizontal: theme.spacing.sm, },
    inputLangText: { ...theme.typography.caption, color: theme.colors.primaryOrange, fontWeight: 'bold', textTransform: 'uppercase', },
    textInputArea: { ...theme.typography.h3, color: theme.colors.darkGray, minHeight: 100, textAlignVertical: 'top', padding: theme.spacing.sm, marginBottom: theme.spacing.sm, },
    clearInputButton: { position: 'absolute', top: theme.spacing.lg + 94, right: theme.spacing.md, },
    activityIndicator: { marginVertical: theme.spacing.md, },
    resultDisplayCard: { backgroundColor: theme.colors.lightGray, borderRadius: theme.borders.radiusMedium, padding: theme.spacing.md, marginTop: theme.spacing.md, },
    resultHeaderActions: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: theme.spacing.sm, },
    resultLangText: { ...theme.typography.caption, color: theme.colors.primaryOrange, fontWeight: 'bold', textTransform: 'uppercase', },
    ...
    actionButtonsContainer: { flexDirection: 'row', justifyContent: 'space-around', marginHorizontal: theme.spacing.md, marginTop: theme.spacing.lg, marginBottom: theme.spacing.md, },
    actionButton: { backgroundColor: theme.colors.white, paddingVertical: theme.spacing.sm, paddingHorizontal: theme.spacing.sm, borderRadius: theme.borders.radiusLarge, alignItems: 'center', justifyContent: 'center', minWidth: 100, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 2, elevation: 2, color: theme.colors.primaryOrange, },
    actionButtonText: { ...theme.typography.button, color: theme.colors.primaryOrange, marginTop: theme.spacing.xs, },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', },
    modalContent: { backgroundColor: theme.colors.white, borderRadius: theme.borders.radiusLarge, padding: theme.spacing.lg, width: '90%', maxHeight: '80%', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5, },
    modalTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 20, textAlign: 'center' },
    pickerContainer: { borderWidth: 1, borderColor: '#ddd', borderRadius: 5, marginBottom: 15 },
