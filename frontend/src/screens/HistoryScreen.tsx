...
const HistoryScreen: React.FC = () => {
  ...
  const [itemToSave, setItemToSave] = useState<LocalTranslationItem | null>(
    null
  );
  ...
  const mapApiToLocal = (
    apiItem: ApiTranslationResponse
  ): LocalTranslationItem => ({
    id: apiItem.id,
  ...
  const loadHistory = useCallback(async () => {
    ...
    setError(null);
    try {
      const response: Page<ApiTranslationResponse> = await apiGetTranslations(
        0,
        100
      );
  ...
  if (error && historyItems.length === 0) {
    return (
      <View style={styles.centered}>
        ...
        <Button title="Retry" onPress={loadHistory} />
      </View>
    );
  }
