...
export const createFolderOptimistic = async (
  name: string
): Promise<FolderItem> => {
  // 1. Create a local-only folder object with a temporary ID.
  ...
  apiCreateFolder({ name })
    .then(async (syncedFolder) => {
      // SUCCESS: Backend returned the real folder data.
      const finalFolder: FolderItem = {
        ...newFolder,
        id: syncedFolder.id,
        isSynced: true,
...
export const saveItemToFolder = async (
  item: TranslationResponse,
  folderId: string
  ...
  const newItemData: TranslationItem = {
    ...
    isSaved: true,
    isFavorite:
      existingIndex > -1 ? allItems[existingIndex].isFavorite : item.isFavorite,
    inputType: item.inputType,
  };
...
export const toggleFavorite = async (
  item: TranslationResponse | TranslationItem
): Promise<TranslationItem> => {
  const allItems = await getSavedItems();
  const existingIndex = allItems.findIndex((i) => i.id === item.id);
  const isCurrentlyFavorite =
    existingIndex > -1
      ? allItems[existingIndex].isFavorite
      : "isFavorite" in item
        ? item.isFavorite
        : false;
  ...
  const updatedItem: TranslationItem = {
    ...
    translatedText:
      "targetText" in item ? item.targetText : item.translatedText,
    sourceLang: item.sourceLang,
    targetLang: item.targetLang,
    timestamp: Date.now(),
